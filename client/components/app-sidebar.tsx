...
export function AppSidebar() {
  ...
  return (
    <aside className="w-full md:w-72 h-screen bg-white border-r flex flex-col">
      {/* User Header */}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`truncate uppercase ${isActive ? "text-white" : "text-gray-900"}`}
                      >
                        {chat.type === "group"
                          ? chat.groupName
                          : chat.participants.find((p) => p._id !== user?._id)
                      ...
                      <p
                        className={`text-xs truncate ${isActive ? "text-blue-100" : unread ? "text-blue-600" : "text-gray-500"}`}
                      >
                        {isDeleted
                          ? "message deleted"
      ...
      <div className="border-t p-3 space-y-2">
        <Drawer direction="left">
          <DrawerTrigger asChild>
            <button className="w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-900 transition-colors">
              <Settings className="h-4 w-4" />
              Settings
            </button>
          </DrawerTrigger>

          <DrawerContent className="h-full w-[300px] p-0 flex flex-col">
            {/* Header */}
              ...
              {settings.map((item) => (
                <div key={item.label} onClick={item.onClick}>
                  {item.modal ? (
                    ...
                    <Link
                      href={item.href || "#"}
                      className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors group"
                    >
                      <span className="w-8 h-8 rounded-md bg-gray-100 group-hover:bg-blue-100 flex items-center justify-center transition-colors">
                        {item.icon}
                      </span>
            ...
            {/* Footer */}
            <DrawerFooter className="px-4 py-4 border-t bg-gray-50">
              <DrawerClose asChild>
                <Button
                  variant="outline"
